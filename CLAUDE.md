...
- **Auth:** Adobe OAuth Server-to-Server (`ORG_ID`, `CLIENT_ID`, `SECRET`, `SCOPES`). Same env-var contract as `cja_auto_sdr`. JWT auth via `aanalytics2` is fully deprecated as of 2025-01-01 — OAuth S2S is the only supported path. The `SCOPES` value must include `additional_info.job_function` or read endpoints return empty/403. Bootstrap is two-step: `aanalytics2.configure(**creds) → Login().getCompanyId() → Analytics(globalCompanyId)`. The integration must also be added to an Adobe Analytics Product Profile in Admin Console for any data to be visible.
