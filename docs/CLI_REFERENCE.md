...
| `--fail-fast` | Stop the batch at the first failure (opt-out of continue-on-error default). Sequential runs stop submitting; parallel runs cancel pending workers. Unattempted RSIDs are recorded as cancelled failures. |
